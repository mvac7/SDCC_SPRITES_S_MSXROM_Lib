...
const char text01[] = "Test SDCC SPRITES S Lib"; 
const char text02[] = "        v1.0 (23/04/2018)";
...
void testSPRITES()
{
  ...
  setSprites16x16Patterns();
  initSprites();
  VPRINT(0,posY++, "PUTSPRITE(plane,x,y,color,nPat)");
  showSprites();                                                                           
 
  WAIT(50);
  posY++;
  
  VPRINT(0,posY++, "--------functions with PUTSPRITE");
  ...
  PUTSPRITE(7, spr_posX[7], spr_posY[7], 8, 7);
  WAIT(50);
...
{
  HALT;
  CopyToVRAM((uint) SPRITE_DATA16x16,BASE14,32*12);
}
...
void showSprites()
{
  ...
  for(i=0;i<8;i++)
  {
    PUTSPRITE(i, spr_posX[i], spr_posY[i], sprcol[i], i);
  }
}
...
void testSpriteVisible()
{
  ...
  for(o=0;o<8;o++)
  {
    for(i=0;i<8;i++)
    {
      if (i==o) PUTSPRITE(i, spr_posX[i], spr_posY[i], sprcol[i], i);
      else PUTSPRITE(i, 0, YHIDDEN, 0, 0);  
    }
    WAIT(25);  
  }  
...
void testSpriteColor()
{
  ...
  for(i=0;i<16;i++)
  {
    PUTSPRITE(7, spr_posX[7], spr_posY[7], i, 7);
    WAIT(25);  
  }  
...
void testSpritePosition()
{
  ...
  {
    HALT;
    spr_posX[7] = posX[posT];
    spr_posY[7] = posY[posT]-32;
    PUTSPRITE(7, spr_posX[7], spr_posY[7], 8, 7);
    posT++;
    if(posT>255) posT=0;  
  }
